...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("queueDiscType", "Set Queue disc type to RED or PDRED", queueDiscType);
  ...
  if ((queueDiscType != "RED") && (queueDiscType != "PDRED"))
    {
      std::cout << "Invalid queue disc type: Use --queueDiscType=RED or --queueDiscType=PDRED" << std::endl;
      exit (1);
    }
  ...
  if (queueDiscType == "PDRED")
    {
      // Turn on ARED
      Config::SetDefault ("ns3::RedQueueDisc::PDRED", BooleanValue (true));
